document.addEventListener("DOMContentLoaded", () => {
    ...
    function displayNewMessages(messages) {
        ...
        messages.forEach(message => {
            ...
            if (!loadedMessages[messageID] && chatID === message.chatid) {
                createMessageElement(sender, messageText, messageID, imagePath,timestamp,avatarPath,nameColor,isFromOwner);
    ...
    function createMessageElement(sender, messageText, messageID, imagePath, timestamp,avatarPath,nameColor,isFromOwner) {
        ...
        if (sender.toLowerCase() === currentUser.toLowerCase()) {
            ...
            newMessage.style.background = 'linear-gradient(332deg, rgba(0,99,255,1) 0%, rgba(0,193,204,1) 100%)';
    ...
    function clearMessages() {
        if (owner == username){
            socket.send(JSON.stringify({ type: 'clearMessages', chatid: getCookie('SelectedChat') }));
            textArea.value = '';
        }
    }
...
function clickChatSettings(event) {
    ...
    function ChatCreate(event) {
        ...
        socket.send(JSON.stringify({ type: 'loadMessages', chatid: getCookie('SelectedChat') }));
        document.querySelectorAll('.messageLine').forEach(element => {
            element.remove();
        })
        loadedMessages = {};
    ...
    // Event Listeners
            document.getElementById('messageForm').addEventListener('submit',(event) => {
                ...
                document.getElementById('pfpChangeForm').addEventListener('submit',(event) => {    
                        event.preventDefault();
                ...
                document.getElementById('nameChangeForm').addEventListener('submit',(event) => {    
                        event.preventDefault();
                       ...
                       else
                       {
                        ...
                        document.getElementById('colorChangeForm').addEventListener('submit',(event) => {    
                        event.preventDefault();
                        ...
                        document.getElementById('chatNameChangeForm').addEventListener('submit',(event) => {    
                            if (owner == username){
                            event.preventDefault();
                            if (document.getElementById('chatNameChangeInput').value.length < 30)
                            {
                            let newChatName = document.getElementById('chatNameChangeInput').value;
                            const formData = new FormData();
                            formData.append('oldChatName', document.getElementById('chatTitle').textContent);
                           ...
                           document.getElementById('chatIconChangeForm').addEventListener('submit',(event) => {    
                           ...
                           document.getElementById('chatDeleteForm').addEventListener('submit',(event) => {    
                            if (owner == username){
                            event.preventDefault();
                            socket.send(JSON.stringify({
                                type: 'chatDelete',
                           ...
                           document.getElementById('chatPasswordChangeForm').addEventListener('submit',(event) => {    
                            event.preventDefault();
                            let newPassword = document.getElementById('chatPasswordChangeInput').value;
                            socket.send(JSON.stringify({
                                type: 'changePassword',
                                newPassword,
                                chatid: getCookie('SelectedChat')
    ...
    document.getElementById('chatSettings').addEventListener('click', clickChatSettings);
    document.getElementById('Logout').addEventListener('click', showLogoutPopup);
    document.getElementById('pfpChange').addEventListener('click', pfpChangePopup)
    document.getElementById('nameChange').addEventListener('click', nameChangePopup)
    document.getElementById('colorChange').addEventListener('click', colorChangePopup)
    document.getElementById('chatPasswordChange').addEventListener('click', chatPasswordChangePopup)
    document.getElementById('chatIconChange').addEventListener('click', chatIconChangePopup)
    document.getElementById('chatNameChange').addEventListener('click', chatNameChangePopup)
    document.getElementById('chatDelete').addEventListener('click', chatDeletePopup)
    ...
    document.querySelectorAll(".nie").forEach(no => {no.addEventListener('click', () => {
    ...
    function isClickOutsideMenu(event) {
        ...
        return !Object.values(menuBoundaries).some(boundary => 
    ...
    messageMenu.addEventListener('mouseleave', () => messageMenu.style.display = 'none');
    accountMenu.addEventListener('mouseleave', () => accountMenu.style.display = 'none');
    chatSettings.addEventListener('mouseleave', () => chatSettings.style.display = 'none');
    ...
    ChatList.addEventListener('click', (event) => {
        if (event.target.dataset.chatID) {
            ...
            if (chatElement) {
                const chatNameElementValue = chatElement.querySelector('.ChatName').textContent;
